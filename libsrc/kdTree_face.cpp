...
namespace trimesh {
	...
	public:
		...
		struct Traversal_Info {
			...
			const KDtreeFace::CompatFunc *iscompat;
			size_t k;
	...
	PoolAlloc KDtreeFace::Node::memPool(sizeof(KDtreeFace::Node));


	// Create a KD tree from the points pointed to by the array pts
	KDtreeFace::Node::Node(const float **pts, size_t n)
	{
		// Leaf nodes
	...
	// Crawl the KD tree
	void KDtreeFace::Node::find_closest_to_pt(KDtreeFace::Node::Traversal_Info &ti) const
	{
		// Leaf nodes
		if (npts) {
	...
	void KDtreeFace::Node::find_k_closest_to_pt(KDtreeFace::Node::Traversal_Info &ti) const
	{
		// Leaf nodes
		if (npts) {
			for (int i = 0; i < npts; i++) {
	...
	void KDtreeFace::Node::find_closest_to_ray(KDtreeFace::Node::Traversal_Info &ti) const
	{
		// Leaf nodes
		if (npts) {
			for (int i = 0; i < npts; i++) {
	...
	void KDtreeFace::build(const float *ptlist, size_t n)
	{
	...
	const float *KDtreeFace::closest_to_pt(const float *p, float maxdist2 /* = 0.0f */,
	...
	const float *KDtreeFace::closest_to_ray(const float *p, const float *dir,
		float maxdist2 /* = 0.0f */,
		const CompatFunc *iscompat /* = NULL */) const
	{
		Node::Traversal_Info ti;
	...
	void KDtreeFace::find_k_closest_to_pt(std::vector<const float *> &knn,
		int k,
		const float *p,
		float maxdist2 /* = 0.0f */,
		const CompatFunc *iscompat /* = NULL */) const
